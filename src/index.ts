...
const LICENSE_MAP: Record<string, keyof LicenseCategory> = {
  MIT: "permissive",
...
function classifyLicense(license: string | null): keyof LicenseCategory {
  if (!license) return "unknown";
  const normalized = license.trim();
  if (LICENSE_MAP[normalized]) return LICENSE_MAP[normalized];

  const lower = normalized.toLowerCase();
  if (/^mit\b/i.test(lower)) return "permissive";
  if (/^apache\b.*2/i.test(lower)) return "permissive";
  if (/^bsd\b/i.test(lower)) return "permissive";
  if (/^isc\b/i.test(lower)) return "permissive";
  if (/\bgpl\b/i.test(lower)) return "copyleft";
  if (/\bagpl\b/i.test(lower)) return "copyleft";
  if (/\bsspl\b/i.test(lower)) return "copyleft";
  if (/\blgpl\b/i.test(lower)) return "weakCopyleft";
  if (/\bmpl\b/i.test(lower)) return "weakCopyleft";
  if (/\bepl\b/i.test(lower)) return "weakCopyleft";
  if (/\bcddl\b/i.test(lower)) return "weakCopyleft";
  if (/\bnc\b/i.test(lower) || /\bnd\b/i.test(lower) || /proprietary/i.test(lower) || /commercial/i.test(lower)) return "proprietary";
  if (/public\s*domain/i.test(lower) || /\bpd\b/i.test(lower) || /\bunlicense\b/i.test(lower)) return "publicDomain";
...
function resolveLicenseFile(pkgPath: string): string | null {
  const candidates = [
  ...
  for (const f of candidates) {
    if (existsSync(join(pkgPath, f))) return f;
  }
  return null;
}

function detectLicenseFromFile(pkgPath: string, file: string): string | null {
  try {
    const content = readFileSync(join(pkgPath, file), "utf-8").slice(0, 2000).toLowerCase();
    if (content.includes("permission is hereby granted, free of charge")) return "MIT";
    ...
    if (content.includes("redistribution and use in source and binary")) {
      if (content.includes("may be used to endorse")) return "BSD-3-Clause";
      return "BSD-2-Clause";
    }
    if (content.includes("isc license") || content.includes("isc license")) return "ISC";
    ...
    if (content.includes("gnu affero general public license")) return "AGPL";
    if (content.includes("mozilla public license")) return "MPL-2.0";
    if (content.includes("this is free and unencumbered software released into the public domain")) return "Unlicense";
  } catch { /* ignore */ }
  return null;
}
...
export function scan(cwd?: string): ScanResult {
  ...
  try {
    npmOutput = execSync("npm ls --json --all --long 2>/dev/null", {
      cwd: dir,
      encoding: "utf-8",
  ...
  function walkDeps(deps: Record<string, any>, parentPath?: string) {
    for (const [name, info] of Object.entries(deps)) {
      ...
      const pkgPath = info.path || (info.resolved ? join(dir, "node_modules", name) : "");
      let license: string | null = info.license ?? null;
      ...
      if (info.dependencies) walkDeps(info.dependencies, pkgPath);
    }
  }

  // Root package
  if (tree.name && tree.version) {
    let rootLicense: string | null = tree.license ?? null;
    if (!rootLicense) {
      try {
  ...
  if (tree.dependencies) walkDeps(tree.dependencies);
...
export function checkPolicy(result: ScanResult, policy: PolicyRule): Violation[] {
  ...
  for (const entry of result.entries) {
    ...
    if (denySet.size > 0) {
      ...
      if (violations.find((v) => v.package === entry.name && v.severity === "error")) continue;
    }
    ...
    if (allowSet.size > 0) {
      ...
      for (const a of allowSet) {
        if (lic.includes(a) || a.includes(lic)) { allowed = true; break; }
      }
      if (!allowed) {
        violations.push({
          package: entry.name,
...
export function formatViolations(violations: Violation[]): string {
  if (violations.length === 0) return "✅ No license violations found.";
...
export function formatJson(result: ScanResult, violations?: Violation[]): string {
  return JSON.stringify({
    stats: result.stats,
    categories: Object.fromEntries(
      Object.entries(result.categories).map(([k, v]) => [k, v.length])
    ),
...
export function formatMarkdown(result: ScanResult, violations?: Violation[]): string {
  const lines: string[] = [];
